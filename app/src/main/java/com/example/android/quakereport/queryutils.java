...
public final class queryutils {
    ...
    */
    ...
    public static ArrayList<Earthquake> extractJsonEarthquakes(String jsonresponse){
        ArrayList<Earthquake> mylist = new ArrayList<>();
        try{
            SimpleDateFormat dateFormatObj = new SimpleDateFormat("MMM dd, yyyy");
            SimpleDateFormat timeFormatObj = new SimpleDateFormat("h:mm a");
            DecimalFormat formater = new DecimalFormat("0.00");
            JSONObject root = new JSONObject(jsonresponse);
            JSONArray feature = root.getJSONArray("features");
            for(int i=0;i<feature.length();i++){

                JSONObject property = feature.getJSONObject(i).getJSONObject("properties");

                Date dateObj = new Date(property.getLong("time"));

                String date = dateFormatObj.format(dateObj);
                String time = timeFormatObj.format(dateObj);
                String magnitude = formater.format(property.getDouble("mag"));
                String place = property.getString("place");
                String url = property.getString("url");


                mylist.add(new Earthquake(date,time,magnitude,place,url));
            }
    ...
    private static URL makeurl(String address) throws MalformedURLException{
        if(address == null)
            return null;
    ...
    private static String getJsonData(InputStream is){
        ...
        try{
            ...
            Log.i("Exception","getjson method raise this exception"+e);
        }catch (IOException e){
            Log.i("Exception","getjson method raise this exception"+e);
        }finally {
    ...
    private static String makeHttpRequest(URL url){
        ...
        try{
            ...
            urlconn.setConnectTimeout(10000 /*mille second */);
            urlconn.setReadTimeout(15000 /*mille second */);
            urlconn.connect();
            if(urlconn.getResponseCode() == HttpURLConnection.HTTP_OK){
                is = urlconn.getInputStream();
                jsonresponse = getJsonData(is);
            }

        }catch (IOException e){
            Log.i("Exception","makeHttp method raise this exception "+e);
        }finally {
            if(urlconn != null) {
                urlconn.disconnect();
                urlconn = null;
    ...
    public static ArrayList<Earthquake> extractEarthquake(String address){



        String jsonresponse = null;
        ArrayList<Earthquake>mylist = null;
        try{
            URL url = makeurl(address);
            jsonresponse = makeHttpRequest(url);
            mylist =  extractJsonEarthquakes(jsonresponse);

        }catch(MalformedURLException e){
            Log.i("Exception :","URL object Exception in extractEarthquake Method "+e);
        }catch (NullPointerException e){
            Log.i("Exception:","NUll pointer Exception in extractEarthquake Method "+e);
        }
        return mylist;
    }
